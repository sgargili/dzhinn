...
import static imf.core.csv.CsvWriter.Letters.*;
...
public class CsvWriter {
    ...
    public void write(String content, boolean preserveSpaces)
        ...
        if (!textQualify
                ...
                || (!useCustomRecordDelimiter && (content.indexOf(LF) > -1 || content.indexOf(CR) > -1))
        ...
        if (textQualify) {
            ...
            if (userSettings.EscapeMode == ESCAPE_MODE_BACKSLASH) {
                content = replace(content, "" + BACKSLASH, ""
                        + BACKSLASH + BACKSLASH);
            ...
            if (useCustomRecordDelimiter) {
                ...
                content = replace(content, "" + CR, ""
                        + BACKSLASH + CR);
                content = replace(content, "" + LF, ""
                        + BACKSLASH + LF);
            }
