...
djd43.provide("djd43.widget.YahooMap");
djd43.require("djd43.event.*");
djd43.require("djd43.math");
djd43.require("djd43.widget.*");
djd43.require("djd43.widget.HtmlWidget");
(function () {
	var yappid = djd43_config["yAppId"] || djd43_config["yahooAppId"] || "dojotoolkit";
	if (!djd43.hostenv.post_load_) {
		if (yappid == "dojotoolkit") {
			djd43.debug("please provide a unique Yahoo App ID in djd43_config.yahooAppId when using the map widget");
		}
		...
		djd43.debug("cannot initialize map system after the page has been loaded! Please either manually include the script block provided by Yahoo in your page or require() the YahooMap widget before onload has fired");
	}
})();
djd43.widget.defineWidget("djd43.widget.YahooMap", djd43.widget.HtmlWidget, function () {
	this.map = null;
	this.datasrc = "";
	this.data = [];
	this.width = 0;
	...
	cLat = djd43.math.round((minLat + maxLat) / 2, 6);
	cLon = djd43.math.round((minLon + maxLon) / 2, 6);
	...
	if (!YMap || !YGeoPoint) {
		djd43.raise("djd43.widget.YahooMap: The Yahoo Map script must be included in order to use this widget.");
	}
	if (this.datasrc) {
		this.parse(djd43.byId(this.datasrc));
	} else {
