...
djd43.provide("djd43.widget.ContentPane");
...
djd43.widget.defineWidget("djd43.widget.ContentPane", djd43.widget.HtmlWidget, function () {
	this._styleNodes = [];
	this._onLoadStack = [];
	this._onUnloadStack = [];
	...
	djd43.widget.ContentPane.superclass.show.call(this);
}, refresh:function () {
	this.isLoaded = false;
	this.loadContents();
	...
	var self = this;
	this._ioBindObj = djd43.io.bind(this._cacheSetting({url:url, mimetype:"text/html", handler:function (type, data, xhr) {
	...
	for (var x in this.bindArgs) {
		if (djd43.lang.isUndefined(bindObj[x])) {
			bindObj[x] = this.bindArgs[x];
		}
	}
	if (djd43.lang.isUndefined(bindObj.useCache)) {
		bindObj.useCache = useCache;
	}
	if (djd43.lang.isUndefined(bindObj.preventCache)) {
		bindObj.preventCache = !useCache;
	}
	if (djd43.lang.isUndefined(bindObj.mimetype)) {
	...
	djd43.deprecated(this.widgetType + ".onUnLoad, use .onUnload (lowercased load)", 0.5);
}, onUnload:function (e) {
	this._runStack("_onUnloadStack");
	delete this.scriptScope;
	if (this.onUnLoad !== djd43.widget.ContentPane.prototype.onUnLoad) {
	...
	djd43.deprecated(this.widgetType + ".addOnUnLoad, use addOnUnload instead. (lowercased Load)", 0.5);
	...
	this.onUnload();
	djd43.widget.ContentPane.superclass.destroy.call(this);
	...
	if (s) {
		...
		var regexInvalid = /(?:var )?\bdjd43_config\b(?:[\s]*=[\s]*\{[^}]+\}|\.[\w]*[\s]*=[\s]*[^;\n]*)?;?|dojo\.hostenv\.writeIncludes\(\s*\);?/g;
			...
			if (this.executeScripts && match[1]) {
				if (attr = regexSrc.exec(match[1])) {
					if (regexDojoJs.exec(attr[2])) {
						djd43.debug("Security note! inhibit:" + attr[2] + " from  being loaded again.");
					} else {
		...
		if (this.executeScripts && this.scriptSeparation) {
			...
			while (tag = regex.exec(s)) {
				...
				while (attr = regexAttr.exec(tag[2])) {
					tag[2] = tag[2].substring(0, attr.index) + attr[1] + "djd43.widget.byId(" + tmp + this.widgetId + tmp + ").scriptScope" + attr[2];
				}
				str += tag[2];
	...
	try {
		...
		while (node.firstChild) {
			djd43.html.destroyNode(node.firstChild);
		}
	...
	if (!data || djd43.html.isNode(data)) {
		...
		for (var i = 0; i < data.styles.length; i++) {
			if (data.styles[i].path) {
				this._styleNodes.push(djd43.html.insertCssFile(data.styles[i].path, djd43.doc(), false, true));
			} else {
				this._styleNodes.push(djd43.html.insertCssText(data.styles[i]));
			}
		}
		...
		function asyncParse() {
			...
			if (_self.parseContent) {
				...
				djd43.widget.getParser().createSubComponents(frag, _self);
			}
			_self.onResized();
			_self.onLoad();
		}
		if (djd43.hostenv.isXDomain && data.requires.length) {
			djd43.addOnLoad(asyncParse);
		} else {
			asyncParse();
		}
	}
	...
	for (var i = 0; i < scripts.length; i++) {
		if (scripts[i].path) {
			djd43.io.bind(this._cacheSetting({"url":scripts[i].path, "load":function (type, scriptStr) {
				djd43.lang.hitch(self, tmp = ";" + scriptStr);
