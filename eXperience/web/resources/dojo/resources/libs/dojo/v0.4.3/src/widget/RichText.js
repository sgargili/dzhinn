...
djd43.provide("djd43.widget.RichText");
...
if (!djd43_config["useXDomain"] || djd43_config["allowXdRichTextSave"]) {
	if (djd43.hostenv.post_load_) {
		(function () {
			var savetextarea = djd43.doc().createElement("textarea");
			savetextarea.id = "djd43.widget.RichText.savedContent";
			savetextarea.style = "display:none;position:absolute;top:-100px;left:-100px;height:3px;width:3px;overflow:hidden;";
			djd43.body().appendChild(savetextarea);
		})();
	} else {
		try {
			djd43.doc().write("<textarea id=\"djd43.widget.RichText.savedContent\" " + "style=\"display:none;position:absolute;top:-100px;left:-100px;height:3px;width:3px;overflow:hidden;\"></textarea>");
		}
		catch (e) {
		}
	}
}
djd43.widget.defineWidget("djd43.widget.RichText", djd43.widget.HtmlWidget, function () {
	...
	this._keyHandlers = {};
	if (djd43.Deferred) {
		this.onLoadDeferred = new djd43.Deferred();
	}
	...
	djd43.event.topic.publish("djd43.widget.RichText::init", this);
	this.open();
	djd43.event.connect(this, "onKeyPressed", this, "afterKeyPress");
	djd43.event.connect(this, "onKeyPress", this, "keyPress");
	djd43.event.connect(this, "onKeyDown", this, "keyDown");
	djd43.event.connect(this, "onKeyUp", this, "keyUp");
	...
	djd43.event.topic.publish("djd43.widget.RichText::open", this);
	this._content = "";
	...
	if ((this.domNode["nodeName"]) && (this.domNode.nodeName.toLowerCase() == "textarea")) {
		...
		this.domNode = djd43.doc().createElement("div");
		djd43.html.copyStyle(this.domNode, this.textarea);
		var tmpFunc = djd43.lang.hitch(this, function () {
		...
		if (!h.safari) {
			djd43.html.insertBefore(this.domNode, this.textarea);
		}
		if (this.textarea.form) {
			djd43.event.connect("before", this.textarea.form, "onsubmit", djd43.lang.hitch(this, function () {
				this.textarea.value = this.getEditorContent();
			}));
		}
		var editor = this;
		djd43.event.connect(this, "postCreate", function () {
			djd43.html.insertAfter(editor.textarea, editor.domNode);
		});
	} else {
		var html = this._preFilterContent(djd43.string.trim(this.domNode.innerHTML));
	}
	if (html == "") {
		html = "&nbsp;";
	}
	var content = djd43.html.getContentBox(this.domNode);
	...
	this.editingArea = djd43.doc().createElement("div");
	this.domNode.appendChild(this.editingArea);
	if ((this.domNode["nodeName"]) && (this.domNode.nodeName == "LI")) {
		this.domNode.innerHTML = " <br>";
	}
	if (this.saveName != "" && (!djd43_config["useXDomain"] || djd43_config["allowXdRichTextSave"])) {
		var saveTextarea = djd43.doc().getElementById("djd43.widget.RichText.savedContent");
		...
		djd43.event.connect("before", window, "onunload", this, "_saveContent");
	}
	if (h.ie70 && this.useActiveX) {
		djd43.debug("activeX in ie70 is not currently supported, useActiveX is ignored for now.");
	...
	if (this.useActiveX && h.ie) {
		...
		if (h.ie || this._safariIsLeopard() || h.opera) {
			this.iframe = djd43.doc().createElement("iframe");
			...
			while (node) {
				djd43.withGlobal(this.window, "selectElement", djd43.html.selection, [node.firstChild]);
			...
			djd43.lang.forEach(this.events, function (e) {
				djd43.event.connect(this.editNode, e.toLowerCase(), this, e);
			}, this);
			this.onLoad();
		} else {
	...
	djd43.html.addClass(this.domNode, "RichTextEditable");
	this.isClosed = false;
}, _hasCollapseableMargin:function (element, side) {
	if (djd43.html.getPixelValue(element, "border-" + side + "-width", false)) {
		return false;
	} else {
		if (djd43.html.getPixelValue(element, "padding-" + side, false)) {
			return false;
		} else {
			return true;
		}
	...
	var elementMargin = djd43.html.getPixelValue(element, marginProp, false);
	function isSignificantNode(element) {
		return !(element.nodeType == 3 && djd43.string.isBlank(element.data)) && djd43.html.getStyle(element, "display") != "none" && !djd43.html.isPositionAbsolute(element);
	}
	var childMargin = 0;
	...
	while (child) {
		...
		childMargin = Math.max(childMargin, djd43.html.getPixelValue(child, marginProp, false));
	...
	while (sibling) {
		if (isSignificantNode(sibling)) {
			contextMargin = djd43.html.getPixelValue(sibling, siblingMarginProp, false);
			break;
		}
		sibling = sibling[siblingAttr];
	}
	if (!sibling) {
		contextMargin = djd43.html.getPixelValue(element.parentNode, marginProp, false);
	}
	...
	var oldMoz = Boolean(djd43.render.html.moz && (typeof window.XML == "undefined"));
	if (!this.iframe) {
		var currentDomain = (new djd43.uri.Uri(djd43.doc().location)).host;
		this.iframe = djd43.doc().createElement("iframe");
		with (this.iframe) {
			style.border = "none";
			style.lineHeight = "0";
	...
	if (djd43_config["useXDomain"] && !djd43_config["dojoRichTextFrameUrl"]) {
		djd43.debug("djd43.widget.RichText: When using cross-domain Dojo builds," + " please save src/widget/templates/richtextframe.html to your domain and set djd43_config.dojoRichTextFrameUrl" + " to the path on your domain to richtextframe.html");
	}
	this.iframe.src = (djd43_config["dojoRichTextFrameUrl"] || djd43.uri.moduleUri("djd43.widget", "templates/richtextframe.html")) + ((djd43.doc().domain != currentDomain) ? ("#" + djd43.doc().domain) : "");
	...
	var tmpContent = djd43.doc().createElement("div");
	tmpContent.innerHTML = html;
	this.editingArea.appendChild(tmpContent);
	if (this.relativeImageUrls) {
		var imgs = tmpContent.getElementsByTagName("img");
		for (var i = 0; i < imgs.length; i++) {
			imgs[i].src = (new djd43.uri.Uri(djd43.global().location, imgs[i].src)).toString();
		}
		html = tmpContent.innerHTML;
	}
	var firstChild = djd43.html.firstElement(tmpContent);
	var lastChild = djd43.html.lastElement(tmpContent);
	if (firstChild) {
	...
	var ifrFunc = djd43.lang.hitch(this, function () {
	...
	if (files.length > 0) {
		for (var i = 0; i < files.length; i++) {
			var url = files[i];
			if (url) {
				this.addStyleSheet(djd43.uri.dojoUri(url));
			}
		}
	}
}, addStyleSheet:function (uri) {
	var url = uri.toString();
	if (djd43.lang.find(this.editingAreaStyleSheets, url) > -1) {
		djd43.debug("djd43.widget.RichText.addStyleSheet: Style sheet " + url + " is already applied to the editing area!");
		return;
	}
	if (url.charAt(0) == "." || (url.charAt(0) != "/" && !uri.host)) {
		url = (new djd43.uri.Uri(djd43.global().location, url)).toString();
	}
	...
	var index = djd43.lang.find(this.editingAreaStyleSheets, url);
	if (index == -1) {
		djd43.debug("djd43.widget.RichText.removeStyleSheet: Style sheet " + url + " is not applied to the editing area so it can not be removed!");
		return;
	}
	...
	for (var i = 0; i < links.length; i++) {
		if (links[i].href == url) {
			...
			djd43.html.removeNode(links[i]);
			break;
		}
	}
}, _drawObject:function (html) {
	this.object = djd43.html.createExternalElement(djd43.doc(), "object");
	with (this.object) {
	...
	this.object.attachEvent("DocumentComplete", djd43.lang.hitch(this, "onLoad"));
	djd43.lang.forEach(this.events, function (e) {
		this.object.attachEvent(e.toLowerCase(), djd43.lang.hitch(this, e));
	}, this);
	...
	if (!this._native2LocalFormatNames["p"]) {
		...
		if (!obj) {
			try {
				obj = djd43.html.createExternalElement(djd43.doc(), "object");
				obj.classid = "clsid:2D360201-FFF5-11D1-8D03-00A0C959BC0A";
				djd43.body().appendChild(obj);
	...
	if (this.object) {
		...
		if (this.iframe && !djd43.render.html.ie) {
			...
			if (djd43.render.html.safari) {
				...
				this.interval = setInterval(djd43.lang.hitch(this, "onDisplayChanged"), 750);
			} else {
				if (djd43.render.html.mozilla || djd43.render.html.opera) {
					var doc = this.document;
					var addListener = djd43.event.browser.addListener;
					var self = this;
					djd43.lang.forEach(this.events, function (e) {
						var l = addListener(self.document, e.substr(2).toLowerCase(), djd43.lang.hitch(self, e));
						if (e == "onBlur") {
							var unBlur = {unBlur:function (e) {
								djd43.event.browser.removeListener(doc, "blur", l);
							}};
							djd43.event.connect("before", self, "close", unBlur, "unBlur");
						}
					});
				}
			}
	...
	if ((!e) && (this.object)) {
		e = djd43.event.browser.fixEvent(this.window.event);
	}
	if ((djd43.render.html.ie) && (e.keyCode == e.KEY_TAB)) {
		e.preventDefault();
		e.stopPropagation();
	...
	djd43.lang.setTimeout(this, this.onKeyPressed, 1, e);
	...
	if ((djd43.render.html.mozilla) && (this._initialFocus)) {
		this._initialFocus = false;
		if (djd43.string.trim(this.editNode.innerHTML) == "&nbsp;") {
	...
	if (this.iframe && !djd43.render.html.ie) {
		...
		if (this.object) {
			...
			if (this.editNode && this.editNode.focus) {
				...
				djd43.debug("Have no idea how to focus into the editor!");
			}
		}
	}
	...
	var gt420 = false;
	if (djd43.render.html.safari) {
		var tmp = djd43.render.html.UA.split("AppleWebKit/")[1];
	...
	return (djd43.render.html.ie && supportedBy.ie) || (djd43.render.html.mozilla && supportedBy.mozilla) || (djd43.render.html.safari && supportedBy.safari) || (gt420 && supportedBy.safari420) || (djd43.render.html.opera && supportedBy.opera);
	...
	if (this.object) {
		...
		if (command == "inserthtml") {
			...
			if ((command == "unlink") && (this.queryCommandEnabled("unlink")) && (djd43.render.html.mozilla)) {
				...
				var a = djd43.withGlobal(this.window, "getAncestorElement", djd43.html.selection, ["a"]);
				djd43.withGlobal(this.window, "selectElement", djd43.html.selection, [a]);
				...
				if ((command == "hilitecolor") && (djd43.render.html.mozilla)) {
					this.document.execCommand("useCSS", false, false);
					returnValue = this.document.execCommand(command, false, argument);
					this.document.execCommand("useCSS", false, true);
				} else {
					if ((djd43.render.html.ie) && ((command == "backcolor") || (command == "forecolor"))) {
	...
	if (this.object) {
		...
		if (djd43.render.html.mozilla) {
			if (command == "unlink") {
				return djd43.withGlobal(this.window, "hasAncestorElement", djd43.html.selection, ["a"]);
			} else {
		...
		var elem = (djd43.render.html.ie) ? this.document.selection.createRange() : this.document;
	...
	if (this.object) {
		if (command == "forecolor") {
			...
			if (command == "backcolor") {
				...
				if (command == "strikethrough") {
					return djd43.withGlobal(this.window, "hasAncestorElement", djd43.html.selection, ["strike"]);
				} else {
					if (command == "inserthorizontalrule") {
						return false;
					}
	...
	if (this.object) {
		...
		if (djd43.render.html.ie && command == "formatblock") {
	...
	this.focus();
	if (djd43.render.html.moz && this.editNode.firstChild && this.editNode.firstChild.nodeType != djd43.dom.TEXT_NODE) {
		djd43.withGlobal(this.window, "selectElementChildren", djd43.html.selection, [this.editNode.firstChild]);
	} else {
		djd43.withGlobal(this.window, "selectElementChildren", djd43.html.selection, [this.editNode]);
	}
	djd43.withGlobal(this.window, "collapse", djd43.html.selection, [true]);
}, placeCursorAtEnd:function () {
	this.focus();
	if (djd43.render.html.moz && this.editNode.lastChild && this.editNode.lastChild.nodeType != djd43.dom.TEXT_NODE) {
		djd43.withGlobal(this.window, "selectElementChildren", djd43.html.selection, [this.editNode.lastChild]);
	} else {
		djd43.withGlobal(this.window, "selectElementChildren", djd43.html.selection, [this.editNode]);
	}
	djd43.withGlobal(this.window, "collapse", djd43.html.selection, [false]);
}, replaceEditorContent:function (html) {
	html = this._preFilterContent(html);
	if (this.isClosed) {
		this.domNode.innerHTML = html;
	} else {
		if (this.window && this.window.getSelection && !djd43.render.html.moz) {
			this.editNode.innerHTML = html;
		} else {
			if ((this.window && this.window.getSelection) || (this.document && this.document.selection)) {
				this.execCommand("selectall");
				if (djd43.render.html.moz && !html) {
					html = "&nbsp;";
				}
	...
	var ec = html;
	djd43.lang.forEach(this.contentPreFilters, function (ef) {
		ec = ef(ec);
	});
	if (this.contentDomPreFilters.length > 0) {
		var dom = djd43.doc().createElement("div");
		dom.style.display = "none";
		djd43.body().appendChild(dom);
		dom.innerHTML = ec;
		djd43.lang.forEach(this.contentDomPreFilters, function (ef) {
			dom = ef(dom);
		});
		ec = dom.innerHTML;
		djd43.body().removeChild(dom);
	}
	return ec;
	...
	if (this.contentDomPostFilters.length > 0) {
		...
		dom.innerHTML = ec;
		djd43.lang.forEach(this.contentDomPostFilters, function (ef) {
			dom = ef(dom);
		});
		ec = dom.innerHTML;
	}
	djd43.lang.forEach(this.contentPostFilters, function (ef) {
		ec = ef(ec);
	});
	return ec;
	...
	var height = djd43.html.getBorderBox(this.editNode).height;
	if (!height) {
		height = djd43.html.getBorderBox(this.document.body).height;
	}
	if (height == 0) {
		djd43.debug("Can not figure out the height of the editing area!");
		return;
	}
	...
	try {
		...
		if (djd43.string.trim(ec) == "&nbsp;") {
			ec = "";
		}
	}
	catch (e) {
	}
	if (djd43.render.html.ie && !this.object) {
		var re = new RegExp("(?:<p>&nbsp;</p>[\n\r]*)+$", "i");
		ec = ec.replace(re, "");
	}
	ec = this._postFilterContent(ec);
	if (this.relativeImageUrls) {
		var siteBase = djd43.global().location.protocol + "//" + djd43.global().location.host;
		var pathBase = djd43.global().location.pathname;
		if (pathBase.match(/\/$/)) {
		} else {
	...
	if (djd43.render.html.ie && !this.object) {
		djd43.event.browser.clean(this.editNode);
	}
	if (this.iframe) {
		delete this.iframe;
	}
	if (this.textarea) {
		...
		djd43.html.removeNode(this.domNode);
		this.domNode = this.textarea;
	} else {
		if (save) {
			if (djd43.render.html.moz) {
				var nc = djd43.doc().createElement("span");
	...
	djd43.html.removeClass(this.domNode, "RichTextEditable");
	this.isClosed = true;
	this.isLoaded = false;
	...
	djd43.widget.RichText.superclass.destroy.call(this);
}, connect:function (targetObj, targetFunc, thisFunc) {
	djd43.event.connect(targetObj, targetFunc, this, thisFunc);
}, disconnect:function (targetObj, targetFunc, thisFunc) {
	djd43.event.disconnect(targetObj, targetFunc, this, thisFunc);
}, disconnectAllWithRoot:function (targetObj) {
	djd43.deprecated("disconnectAllWithRoot", "is deprecated. No need to disconnect manually", "0.5");
