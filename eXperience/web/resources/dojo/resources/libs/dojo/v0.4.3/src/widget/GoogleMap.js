...
djd43.provide("djd43.widget.GoogleMap");
...
(function () {
	var gkey = djd43_config["gMapKey"] || djd43_config["googleMapKey"];
	var uri = new djd43.uri.Uri(window.location.href);
	...
	if (!djd43.hostenv.post_load_) {
		if (!gkey || gkey == "") {
			djd43.raise("djd43.widget.GoogleMap: The Google Map widget requires a proper API key in order to be used.");
		}
		...
		djd43.debug("Cannot initialize Google Map system after the page has been loaded! Please either manually include the script block provided by Google in your page or require() the GoogleMap widget before onload has fired.");
	}
})();
djd43.widget.defineWidget("djd43.widget.GoogleMap", djd43.widget.HtmlWidget, function () {
	this.map = null;
	this.geocoder = null;
	this.data = [];
	this.datasrc = "";
