...
djd43.hostenv.loadPath = function (relpath, module, cb) {
	...
	if (djd43_config.cacheBust && djd43.render.html.capable) {
		uri += "?" + String(djd43_config.cacheBust).replace(/\W+/g, "");
	}
	try {
...
djd43.hostenv.callLoaded = function () {
	if (typeof setTimeout == "object" || (djd43_config["useXDomain"] && djd43.render.html.opera)) {
...
if (djd43_config["modulePaths"]) {
	for (var param in djd43_config["modulePaths"]) {
		djd43.registerModulePath(param, djd43_config["modulePaths"][param]);
	}
}
...
djd43.hostenv.preloadLocalizations = function () {
	if (djd43.hostenv.localesGenerated) {
		...
		preload();
		var extra = djd43_config.extraLocale || [];
...
djd43.requireLocalization = function (moduleName, bundleName, locale, availableFlatLocales) {
	...
	if (bundle) {
		if (djd43_config.localizationComplete && bundle._built) {
			return;
		}
