...
if((typeof ScriptEngineMajorVersion != 'function')||(ScriptEngineMajorVersion() < 7)){
	djd43.raise("attempt to use JScript .NET host environment with inappropriate ScriptEngine"); 
}
...
djd43.hostenv.getText = function(uri){
	if(!System.IO.File.Exists(uri)){
		// djd43.raise("No such file '" + uri + "'");
		return 0;
	}
...
djd43.hostenv.loadUri = function(uri){
	...
	if(!contents){
		djd43.raise("got no back contents from uri '" + uri + "': " + contents);
	}
	// TODO: in JScript .NET, eval will not affect the symbol table of the current code?
	var value = dj_eval(contents);
	djd43.debug("jsc eval of contents returned: ", value);
	return 1;
...
djd43.hostenv.getLibraryScriptUri = function(){
