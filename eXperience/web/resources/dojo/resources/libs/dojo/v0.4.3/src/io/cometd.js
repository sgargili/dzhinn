...
cometd = new function () {
	...
	this.init = function (props, root, bargs) {
		...
		this.url = root || djd43_config["cometdRoot"];
		if (!this.url) {
			djd43.debug("no cometd root specified in djd43_config and no root passed");
			return;
		}
...
cometd.iframeTransport = new function () {
	...
	this.sendMessage = function (message, bypassBacklog) {
		if ((bypassBacklog) || (this.connected)) {
			...
			var bindArgs = {url:cometd.url || djd43_config["cometdRoot"], method:"POST", mimetype:"text/json", content:{message:djd43.json.serialize([message])}};
			return djd43.io.bind(bindArgs);
		} else {
...
cometd.longPollTransport = new function () {
	...
	this.sendMessage = function (message, bypassBacklog) {
		if ((bypassBacklog) || (this.connected)) {
			message.connectionId = this.connectionId;
			message.clientId = cometd.clientId;
			var bindArgs = {url:cometd.url || djd43_config["cometdRoot"], method:"post", mimetype:"text/json", content:{message:djd43.json.serialize([message])}, load:djd43.lang.hitch(this, function (type, data, evt, args) {
				cometd.deliver(data);
			})};
			return djd43.io.bind(bindArgs);
		} else {
...
cometd.callbackPollTransport = new function () {
	...
	this.sendMessage = function (message, bypassBacklog) {
		if ((bypassBacklog) || (this.connected)) {
			message.connectionId = this.connectionId;
			message.clientId = cometd.clientId;
			var bindArgs = {url:cometd.url || djd43_config["cometdRoot"], mimetype:"text/json", transport:"ScriptSrcTransport", jsonParamName:"jsonp", content:{message:djd43.json.serialize([message])}, load:djd43.lang.hitch(this, function (type, data, evt, args) {
				cometd.deliver(data);
			})};
			return djd43.io.bind(bindArgs);
		} else {
